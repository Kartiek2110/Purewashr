            ...
            <div className="md:w-1/2 md:h-full md:flex md:justify-center md:items-center md:relative hidden">
              <div ref={navRef} className="w-[50vh] h-full flex justify-center gap-8 items-center ml-[-15vw] text-md text-white relative">
                {['Home', 'About', 'Pricing', 'Contact'].map((item) => (
                  <Link key={item} to={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className={`relative ${activeSection === item ? "font-bold" : ""} hover:text-yellow-400 transition-colors duration-300`}>
                    {item}
                    <motion.div
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-[#162479] bg-opacity-90 text-white">
          <div className="flex flex-col items-center py-4">
            {['Home', 'About', 'Pricing', 'Contact'].map((item) => (
              <Link key={item} to={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className={`py-2 relative ${activeSection === item ? "font-bold" : ""} hover:text-yellow-400 transition-colors duration-300`} onClick={() => { setActiveSection(item); setIsMenuOpen(false); }}>
                {item}
                <motion.div
                  className="absolute bottom-0 left-0 w-full h-0.5 bg-white"
                  initial={{ width: activeSection === item ? "100%" : "0%" }}
